...
# Lösung: Anfang
C = A.union(B)                  # Alternativ: C = A | B
D = A.intersection(B)           # Alternativ: D = A & B
E = A.difference(B)             # Alternativ: E = A - B
F = A.symmetric_difference(B)   # Alternativ: F = A ^ B
print(f"{A}∪{B}={C}")
print(f"{A}∩{B}={D}")
print(f"{A}∖{B}={E}")
print(f"{A}Δ{B}={F}")
...
# a) Gib die Menge A an, damit die Vereinigung 𝐴∪𝐵 die Zielmenge C ergibt.
# Lösung: Anfang
A = {4,6}
print(f"Menge A={A}")
# Lösung: Ende
...
# a) Gib eine Menge D an, so dass C⊆A wobei C=B\D.
# Lösung: Anfang
D = {5,6}
# Lösung: Ende
